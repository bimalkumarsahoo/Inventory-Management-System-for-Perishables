    ...
    for t = 1:time_periods
        for ret = 1:num_retailers
            ...
            for al = 1:10
                temp = 0;
                for j = 1:10
                    d = demand_intervals(j);
                    prob = probability_matrix(ret, t, d);
                    if t ==  time_periods
                        cost = cost_fun(price, price, d,...
                            0, al);
                    else
                        cost = cost_fun(price, holding_cost, d...
                            , 0, al);
                    end
                    temp = temp + prob*cost;
                end
                if (temp < expected_cost)
                    expected_cost = temp;
                    expected_al = al;
                    retailer_al(ret, t) = expected_al;
                end
            end
        end
    end 
